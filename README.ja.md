...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 074 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
**Panel 074** は、元写真で不可欠な主体、動作、関係、暗喩を見極め、最少の要素で独自の正面擬似3D／2.5Dアイコンへ再建します。外形は常に規格的な1:1角丸正方形で、内部だけが遮蔽、厚み、レリーフ、距離によって一続きの立体空間になります。第一印象で主体、次に関係、最後に感情が伝わる設計です。

### 向いている用途

- 外形は上質な App Icon 規格を守りながら、内部の物語に独自の比喩を持つ、実際にレビュー可能なアイコン概念が必要な場合
- 写真を四角に押し込むのではなく、核となる被写体、動き、関係を再構成して写真の魂を残したい場合
- 実在する厚みと連続空間を持ちながら、プラスチック玩具、ゼリーガラス、ゲーム UI に見えない柔らかな現代的 2.5D を好む場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 074 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 標準角丸正方形 · 正面擬似3D／2.5D · 元写真の魂の抽出 · 連続空間遮蔽 · マット彫刻 · ブランドアイコン提案
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | ブランド視覚提案やアイコン審査に使える独自の角丸擬似3Dアイコン |
| **識別しやすい特徴** | 固定1:1角丸外形の内部に、遮蔽、厚み、レリーフ、距離でつくられた連続正面空間 |
| **入力の尊重** | 主体、輪郭、姿勢、中心関係を保ち、色は元写真だけから導いて再調整する |
...
```text
主体、動作、関係、暗喩を理解 → 標準1:1角丸正方形の舞台を固定 → 最少の重要要素で元写真の魂を再建 → 正面遮蔽、厚み、高低、柔らかな切面、レリーフで連続2.5D空間をつくる → 色を元写真だけから再ディレクション → 柔らかなマット彫刻で仕上げる → 外側に静かな余白と極少の編集文字を置く
```

## 完成作の特徴

- 外形は四辺等長、統一角丸、正面平視の1:1角丸矩形を維持し、主体に変形させない。
- 内部は遮蔽、厚み、高低、柔らかな切面、局部レリーフ、距離による連続正面空間にする。
- 一つの中心的な視覚関係を軸にし、象徴変換しても単なる中央配置の物体にしない。
- 色は元写真だけから導き、固定パレットや平均採色を使わない。
- 質感は繊細、マット、柔らかく、編集イラストと微縮模型とブランド3Dの中間に置く。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-074-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 074 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-074 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-074 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-074 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-074 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 074 を適用し、共通設定は一度だけ解決
/xxd-panel-074 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-074`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-074-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-074-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

74 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 074 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–074） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
