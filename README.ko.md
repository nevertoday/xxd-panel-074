...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 074이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
**Panel 074**는 원본에서 핵심 피사체, 행동, 관계, 은유를 찾아 가장 적은 요소로 독창적인 정면 유사 3D／2.5D 아이콘을 다시 만듭니다. 외곽은 항상 규격화된 1:1 둥근 정사각형이며 내부만 가림, 두께, 부조, 거리로 하나의 연속 입체 공간을 형성합니다. 첫눈에는 피사체, 다음에는 관계, 마지막에는 감정이 읽히도록 설계합니다.

### 이런 경우에 적합합니다

- 외곽은 고급 App Icon 규격을 지키고 내부 서사는 독창적 은유를 지닌, 실제로 심사 가능한 아이콘 콘셉트가 필요할 때
- 사진을 사각형 안에 넣는 대신 핵심 피사체, 행동과 관계를 재구성해 사진의 영혼을 보존하고 싶을 때
- 실제 부피와 연속 공간은 있지만 플라스틱 장난감, 젤리 유리나 게임 UI처럼 보이지 않는 부드러운 현대 2.5D를 좋아할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 074로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 표준 둥근 정사각형 · 정면 유사 3D／2.5D · 원본 영혼 추출 · 연속 공간 가림 · 무광 조각감 · 브랜드 아이콘 제안
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 브랜드 비주얼 제안이나 아이콘 콘셉트 리뷰에 사용할 수 있는 독창적 둥근 유사 3D 아이콘 |
| **대표 특징** | 고정 1:1 둥근 외곽 안에 가림, 두께, 부조, 거리로 만든 연속 정면 공간 |
| **입력 존중 방식** | 피사체, 윤곽, 자세, 핵심 관계를 유지하고 색은 원본 사진에서만 도출해 재조율함 |
...
```text
피사체·행동·관계·은유 이해 → 표준 1:1 둥근 정사각형 무대 고정 → 가장 적은 핵심 요소로 원본 영혼 재구성 → 정면 가림·두께·높낮이·부드러운 절단면·부조로 연속 2.5D 공간 생성 → 색을 원본에서만 재디렉션 → 부드러운 무광 조각감으로 마감 → 바깥에 조용한 여백과 극소량 편집 문자 배치
```

## 완성작의 특징

- 외곽은 네 변이 같고 모서리가 통일된 정면 1:1 둥근 사각형을 유지하며 피사체가 바꾸지 못합니다.
- 내부는 가림, 두께, 높낮이, 부드러운 절단면, 국부 부조, 거리로 만든 연속 정면 공간입니다.
- 하나의 핵심 시각 관계를 중심으로 하며 상징 변환도 평범한 중앙 배치 물체로 퇴화하지 않습니다.
- 색은 원본 사진에서만 도출하며 고정 팔레트나 평균 채색을 사용하지 않습니다.
- 질감은 섬세하고 무광이며 부드럽고 편집 일러스트·미니어처 모델·브랜드 3D 사이에 둡니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-074-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 074의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-074 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-074 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-074 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-074 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-074 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-074 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 074 적용, 공통 설정은 한 번만 확정
/xxd-panel-074 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-074`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-074-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-074-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

74개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 074까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–074) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
