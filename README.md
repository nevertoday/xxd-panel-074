<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 074 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 074 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
理解主体、动作、关系与隐喻 → 固定标准 1:1 圆角正方形舞台 → 用最少关键元素重建原图灵魂 → 以正面遮挡、厚度、高低、柔和切面与局部浮雕构成连续 2.5D 空间 → 完全从原图重新艺术指导颜色 → 以哑光柔和雕塑感完成 → 在图标外保留安静留白与极少编辑文字
```

## 成品中最容易识别的风格特点

- 主体只保留最值得记住的身份、姿态、轮廓与叙事关系，不完整复刻复杂场景。
- 外轮廓始终为四边等长、统一圆角、正面平视的标准 1:1 圆角矩形，不被主体改变。
- 内部是一个连续的正面伪 3D 空间，通过可信遮挡、厚度、高低、柔和切面与局部浮雕建立层次。
- 构图围绕一个核心视觉关系，可夸张比例或象征转换，但不能退化成普通“物体居中”。
- 色彩完全从原图推导并重新调制，不采用固定色板、平均取色或无重点多彩。
- 质感细腻、哑光、柔和，位于高端编辑插画、微型艺术模型和品牌 3D 视觉之间。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-074-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–074） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
